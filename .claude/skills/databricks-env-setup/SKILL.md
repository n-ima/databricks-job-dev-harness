...
## devリソースの初期化（テストフェーズの前提）

Jobの初回実行前に、devターゲットのリソースを初期化する。**Volumeの出力先ディレクトリは
自動では作られない**ため、未作成のままJobを実行すると初回実行が失敗する
（検証プロジェクトで実証済み）。初期化はスクリプト化して再実行可能にする
（例: `scripts/setup_dev_env.py` を実装フェーズで作成し、以下を冪等に行う）。

1. `CREATE SCHEMA IF NOT EXISTS <catalog>.<schema>`
2. `CREATE VOLUME IF NOT EXISTS <catalog>.<schema>.<volume>`
3. Volume配下の入出力ディレクトリ作成（出力先は空でも `.gitkeep` 等で先に掘る）
4. テーブルの `CREATE TABLE IF NOT EXISTS`（設計のDDLどおり）
5. テストデータの投入（`databricks fs cp` でVolumeへ）

認証プロファイル名は環境により `DEFAULT` とは限らない（ホスト名由来のことがある）。
SDK/CLI呼び出しで明示指定が必要な場合は、その値を `environment.md` と
`learnings.md` に記録する。
