...
if [[ "$file" == *_template.md ]]; then
  printf '%s\n' '{"continue": true, "hookSpecificOutput": {"permissionDecision": "deny", "permissionDecisionReason": "テンプレートファイルは直接編集せず、コピーして実体ファイル(例: design_index_template.md -> design-index.md)を作成してください。"}}'
else
  printf '%s\n' '{"continue": true}'
fi
