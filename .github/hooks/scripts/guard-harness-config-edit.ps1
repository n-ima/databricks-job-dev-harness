...
$protectedPattern = '(^|[\\/])\.github[\\/]agents[\\/]|(^|[\\/])\.github[\\/]hooks[\\/]|(^|[\\/])\.github[\\/]workflows[\\/]|(^|[\\/])AGENTS\.md$|(^|[\\/])CLAUDE\.md$|(^|[\\/])GEMINI\.md$|(^|[\\/])plugin\.json$|(^|[\\/])\.vscode[\\/]settings\.json$|(^|[\\/])\.claude[\\/]agents[\\/]|(^|[\\/])\.claude[\\/]commands[\\/]|(^|[\\/])\.claude[\\/]settings\.json$|(^|[\\/])\.agent[\\/]workflows[\\/]'
...
if ($file -and ($file -match $protectedPattern)) {
  $out = @{
    ...
    hookSpecificOutput = @{
      ...
      permissionDecisionReason = "ハーネスの運用ルール自体(agents/hooks/workflows/AGENTS.md/CLAUDE.md/GEMINI.md/plugin.json/settings.json、および.claude・.agentの生成物)はエージェントが自動で書き換えません。変更が必要な場合は.github/側を人間が直接編集し、scripts/generate_agent_tooling.pyを再実行してください。"
    }
  }
} else {
  $out = @{ continue = $true }
