...
# 高確度パターン(Databricks PAT・クラウドの鍵形式・秘密鍵ヘッダ等)はdeny、
# 汎用パターン(api_key=... 等、誤検知しうる)はaskに留める。
input=$(cat)

high_confidence='dapi[0-9a-f]{32}|AKIA[0-9A-Z]{16}|-----BEGIN( RSA| EC| OPENSSH)? PRIVATE KEY-----|gh[pousr]_[A-Za-z0-9]{20,}|xox[baprs]-[A-Za-z0-9-]{10,}'
generic='(api[_-]?key|secret|token|password)\\?["'"'"']?\s*[:=]\s*\\?["'"'"'][A-Za-z0-9/+=_-]{16,}\\?["'"'"']'

if printf '%s' "$input" | grep -Eq "$high_confidence"; then
  printf '%s\n' '{"continue": true, "hookSpecificOutput": {"permissionDecision": "deny", "permissionDecisionReason": "Databricks PAT/クラウド認証情報/秘密鍵とみられる文字列を検出しました。認証情報はコードやドキュメントに直接書かず、environment.mdに記載したシークレット管理先(Databricks Secrets / GitHub Secrets等)を参照してください。"}}'
  exit 0
fi
