# Databricks Job 開発ハーネス — GitHub Copilot / Claude Code / Antigravity対応
...
VS Code + GitHub Copilot / Claude Code / Antigravity で、Databricks Job
（Lakeflow Jobs）の「設計書チェック → 実装計画 → 実装 → テスト → デプロイ/リリース」を
一気通貫でオーケストレーションするための **Databricks特化テンプレート** です。
唯一の正は `.github/` 配下と `AGENTS.md` であり、Claude Code向け（`.claude/`,
`CLAUDE.md`）・Antigravity向け（`.agent/`, `GEMINI.md`）の設定はそこから自動生成する
（詳細は [対応するAIコーディングツール](#対応するaiコーディングツール) を参照）。
...
- 次のいずれかのAIコーディングツール: VS Code + GitHub Copilot Chat拡張機能
  （Custom Agents / Agent Skills対応バージョン）/ Claude Code / Antigravity
...
uv run task gen-tooling-check   # 生成結果と現状の差分を検査(CIでも実行)
```

Claude Codeのサブエージェント（design-critic/task-worker/reviewer）は`Task`ツールで
自動的に呼び出される。Antigravityは本ハーネス作成時点（2026年7月）でSubagents/Hooks機能
の公式スキーマを検証できなかったため、独立レビューはManager Surfaceでの手動起動、
本番保護はTerminal Permission Mode（許可リスト）で代替している（詳細は`GEMINI.md`）。
...
### 起動経路と等価性（プロンプト / ハンドオフ / サブエージェント）

エージェントの起動には「プロンプト実行（`agent:` バインドで自動切替）」
「ハンドオフボタン（遷移先エージェント+短い引き継ぎ文。`.prompt.md` は読まれない）」
「`runSubagent`（design-critic / task-worker / reviewer。親エージェントが自動で呼ぶため
人は起動しない）」の3経路があります。**振る舞いの正は常に `.agent.md`** に置き、
プロンプトは薄い起動ショートカットに留めているため、
**どの経路で起動しても同じ動作になります**（AGENTS.mdの等価性ルール）。
新しいチャット推奨とハンドオフボタンが同時に出た場合、どちらを選んでも結果は同じで、
違いは履歴を引き継ぐこと（コストと文脈汚染）だけです。
...
CLAUDE.md                     … Claude Code用（@AGENTS.mdをインポートするだけ）
GEMINI.md                     … Antigravity固有の補足（AGENTS.mdは直接読まれる）
...
scripts/
  generate_agent_tooling.py   … .github/から.claude/・.agent/を生成する唯一のスクリプト
.github/                      … 【唯一の正】以下から.claude/・.agent/が自動生成される
...
.claude/                      … 【自動生成、直接編集しない】Claude Code向け設定
  agents/                     … サブエージェント（design-critic/task-worker/reviewer）
  commands/                   … スラッシュコマンド（フェーズエージェント人格を内包）
  skills/                     … .github/skills/のコピー
  settings.json               … Hooks + permissions
.agent/                       … 【自動生成、直接編集しない】Antigravity向け設定
  workflows/                  … フェーズ手順（フェーズエージェント人格を内包）
docs/
...
- **Antigravity向けのSubagents/Hooks機能（2026年I/Oで追加）は、本ハーネス作成時点で
  公式ドキュメントのスキーマを検証できなかったため生成対象に含めていない**（`GEMINI.md`参照）。
  Manager Surfaceでの手動起動・Terminal Permission Modeで代替している。公式仕様が
  安定・文書化された場合は `scripts/generate_agent_tooling.py` への追加を検討する。
- Claude Codeのhooks（`.claude/settings.json`）はshell実行に`bash`を前提にしている。
  Windowsでgit-bash/WSLが使えない環境では、該当hookのcommand/shellフィールドを
  PowerShell版（`.github/hooks/scripts/*.ps1`）に読み替えること。
